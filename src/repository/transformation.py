...
async def create_crop_transformation(original_photo, body, user):
    ...
    res_photo = cloudinary.uploader.upload(
        file=original_photo.file_path, public_id=public_id, overwrite=True
    )
    print(res_photo)
    transform = cloudinary.CloudinaryImage(res_photo.get("url")).build_url(
        transformation=[
            {
                "aspect_ratio": f"{body.aspect_ratio}",
                "width": body.width,
                "crop": body.crop,
            },
            {"radius": "max"},
