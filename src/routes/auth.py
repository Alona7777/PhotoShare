...
async def login(
    ...
    if not user.verified:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=auth_massages.EMAIL_NOT_VERIFIED
        )
    ...
    if ban_user:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail=auth_massages.BAN_USER
        )
    # Generate JWT token
...
async def refresh_token(
    ...
    if user.refresh_token != token:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail=auth_massages.INVALID_TOKEN
        )
...
async def verified_email(token: str, db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    """
    ...
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=auth_massages.VERIFICATION_ERROR
        )
    if user.verified:
...
async def reset_password(
    ...
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=auth_massages.RESET_PASSWORD_ERROR
        )
