...
class User(Base) :
    ...
    user_photos: Mapped[relationship] = relationship("Photo", back_populates = "photos_user",
                                                     cascade = "all, delete-orphan")
    user_comments: Mapped[relationship] = relationship("Comment", back_populates = "user",
                                                       cascade = "all, delete-orphan")
    user_ratings: Mapped[relationship] = relationship("Rating", back_populates = "user", cascade = "all, delete-orphan")
...
class Photo(Base) :
    ...
    commented_photos: Mapped[relationship] = relationship("Comment", back_populates = "photo_commented",
                                                          cascade = "all, delete-orphan")
...
class Rating(Base) :
    ...
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey('users.id'))
    photo_id: Mapped[int] = mapped_column(Integer, ForeignKey('photos.id'))
    rating: Mapped[int] = mapped_column(Integer)  # Оцінка користувача фотографії
    created_at: Mapped[date] = mapped_column('created_at', DateTime, default = func.now(), nullable = False)
